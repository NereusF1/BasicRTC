...
It looks like the sub-second ( upto ~1s max ) loss by the HAL_RTC_INIT() has now been fixed by ST. -- or we may be still facing the issue.. but we'll just take the 1s hit for now and proceed. Hopefully we won't see much impact as we are syncing with the Cell Network Time every minute or so.
